...
export function LocationStep({ form, updateForm }: LocationStepProps) {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      const res = await fetch(
        `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(query)}.json?access_token=${token}&proximity=${CHAPMAN_CENTER.lng},${CHAPMAN_CENTER.lat}&limit=5`
      );
      const data = await res.json();
  ...
  const selectSearchResult = (result: {
    ...
    placeMarker(lng, lat);
    updateForm({ address: result.place_name });
    setAddressEditable(false);
    setSearchResults([]);
